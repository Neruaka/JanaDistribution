...
Déploiement : Fly.io (`jana-frontend.fly.dev` / `jana-backend.fly.dev` / Postgres `jana-db`) — EN PRODUCTION depuis 2026-09-04 (Phase 14). Auto-deploy temporaire à chaque push sur `develop` via GitHub Actions (`.github/workflows/deploy-flyio.yml`, décision utilisateur 2026-09-05 — à désactiver une fois le site jugé fini) ; déploiement manuel équivalent : `flyctl deploy` — voir `docs/deploiement/DEPLOY-FLYIO.md`. Railway abandonné (plan expiré) ; le homeserver `tfredklab.dev` (Phase 11) a été supersédé par cette décision utilisateur avant d'être mis en service — ne pas reprendre ce chemin sans confirmation explicite.
