...
| Indicateur | Valeur |
|---|---|
| Phase active | Phase 16 (deuxième phase de tests utilisateur) — exécution EN COURS (voir §5, T16-01..T16-13). T16-03, T16-04, T16-05, T16-06, T16-09, T16-10, T16-11, T16-12 DONE. Phase 15 reste COMPLÈTE (12/12, T15-01..T15-12). |
| Tâche active | Session 2026-09-07 : 13 points remontés, investigués puis plan approuvé. T16-12, le lot T16-03/T16-04/T16-05/T16-06/T16-11, T16-10 et T16-09 (validation comptes pro, checkout bloqué) corrigés et testés, chacun en commit séparé — **T16-09 pas encore déployé en production** (migration 0015 à appliquer). Prochain lot : T16-13 (liste récurrente) puis le lot design (T16-01/02/07/08). |
...
| TODO | 5 (Phase 16 : T16-01, T16-02, T16-07, T16-08, T16-13 — voir §5 pour le détail et les priorités P0/P1/P2/P3) |
| DONE | 131 (110 + 12 Phase 15 + T16-03/T16-04/T16-05/T16-06/T16-09/T16-10/T16-11/T16-12) |
...
- **Détail :** vérifié directement sur Fly.io (`flyctl volumes show`). Le volume Postgres `pg_data` (app `jana-db`) a `Scheduled snapshots: true` (snapshots automatiques quotidiens actifs). Première sauvegarde manuelle déclenchée le 2026-09-05 (`flyctl volumes snapshots create`) pour ne pas dépendre uniquement du premier cycle automatique. **Rétention portée à 30 jours le 2026-09-06** (`flyctl volumes update vol_r1j5xkkojdz03xpr --snapshot-retention 30 --app jana-db`, confirmé via `flyctl volumes show` : `Snapshot retention: 30`) — était à 5 jours (défaut Fly), sous la recommandation de la checklist, demande explicite du propriétaire suite aux retours de test. Stockage images produits sur volume persistant confirmé (`jana_uploads`, app `jana-backend`) ; procédure de restauration toujours non testée (nécessiterait de restaurer un snapshot sur un volume de test, hors scope de cette session).
...
- **Statut :** DONE (2026-09-07) | **Priorité :** P1 | **Catégorie :** CODE (fonctionnalité, terrain vierge)
- **Root cause :** `auth.service.js` validait seulement le format du SIRET (14 chiffres), aucune vérification d'authenticité, et le compte était actif immédiatement. Aucune colonne de statut dédiée sur `utilisateur`. **Décision tranchée avec l'utilisateur avant implémentation :** le checkout est bloqué (pas la connexion) tant qu'un compte pro n'est pas validé.
- **Correctif appliqué :** migration `0015_validation_compte_pro.sql` — colonne `statut_validation_pro` (`NON_APPLICABLE`/`EN_ATTENTE`/`VALIDE`), comptes PROFESSIONNEL déjà existants grandfathered en `VALIDE`. `userRepository.create()` démarre tout nouveau compte PROFESSIONNEL à `EN_ATTENTE`. `order.service.js#createFromCart` rejette la commande (400, message explicite) tant que non validé. Route admin `PATCH /api/admin/clients/:id/valider-pro` (audit log + email de notification). `AdminClientsList.jsx` : badge "En attente", action "Valider ce compte pro" (menu + modale détail), KPI "Pros en attente".
- **Testé :** unitaires (`order.service.test.js`, 3 nouveaux cas) + script E2E direct (inscription pro réelle → checkout bloqué avec le message exact → validation admin → checkout réussi) + vérification navigateur complète (inscription, blocage, badge/KPI admin, validation, déblocage).
- **Fichiers :** `backend/scripts/migrations/0015_validation_compte_pro.sql`, `backend/scripts/init.sql`, `backend/src/repositories/user.repository.js`, `backend/src/services/order.service.js`, `backend/src/routes/admin.clients.routes.js`, `backend/src/services/email.service.js`, `backend/tests/unit/order.service.test.js`, `frontend/src/pages/admin/AdminClientsList.jsx`, `frontend/src/services/adminService.js`.
...
- **Statut :** DONE (2026-09-07) | **Priorité :** P1 | **Catégorie :** CODE (bug)
- **Root cause confirmée :** `frontend/src/pages/LoginPage.jsx:29` — case "Rester connecté sur cet appareil" **décochée par défaut**. `AuthContext.jsx` transmettait ce `rememberMe` tel quel comme argument `persist` à `writeAuthStorage` (`api.js:27-36`) : `persist === false` → écriture dans `sessionStorage` (jamais partagé entre onglets, y compris même origine) au lieu de `localStorage`. `register()` n'avait même pas de case à cocher, retombait sur le même comportement par défaut.
- **Correctif appliqué :** `login()`/`register()` (`AuthContext.jsx`) appellent désormais `setAuthData(..., true)` sans condition — toute connexion persiste en `localStorage`. Case "Rester connecté" retirée de `LoginPage.jsx` (n'avait plus de sens, aurait été un contrôle trompeur). Vérifié en navigateur : inscription réelle → `localStorage.token` présent (`sessionStorage` vide) → nouvel onglet sur `/mon-compte` affiche directement le compte connecté, sans redirection vers `/login`.
- **Fichiers :** `frontend/src/contexts/AuthContext.jsx`, `frontend/src/pages/LoginPage.jsx`

### T16-12 — Code promo absent du devis/facture + affichage de l'usage d'un code promo

- **Statut :** DONE (2026-09-07) | **Priorité :** P0 | **Catégorie :** CODE (bug financier/légal)
- **Root cause confirmée :** `backend/src/services/invoice.service.js#_buildLignesAndTotals` recalculait HT/TVA/TTC en re-sommant les lignes de commande, sans jamais lire `commande.montant_rabais` ni `commande.code_promo_id`. Touchait aussi bien les vraies factures que les devis. Trois totaux différents pour la même commande (devis, facture, `commande.total_ttc`).
- **Correctif appliqué :** `_buildLignesAndTotals` applique désormais le ratio réel de remise (`montant_rabais / total_avant_rabais`) aux totaux HT/TVA des lignes produits, et reprend `total_ttc` directement depuis `commande.total_ttc` (source de vérité, livraison + remise incluses) plutôt que de le re-dériver — garantit l'égalité document/commande même avec livraison payante. La remise (montant complet + code) est stockée en snapshot immuable sur `facture` (migration `0014_facture_remise.sql`, colonnes `remise_montant`/`remise_code`) et affichée sur le PDF (`invoice-pdf.generator.js`). Testé via script E2E (commande + code promo + livraison payante réels en local) : `order.totalTtc === devis.total_ttc === facture.total_ttc` confirmé, plus 2 nouveaux tests unitaires (`invoice.service.test.js`).
- **Affichage de l'usage promo :** ajouté sur `OrderConfirmationPage.jsx` (client) et `AdminOrderDetail.jsx` (admin) — `order.montantRabais`/`codePromoCode` exposés par `order.repository.js` (route client) et par `admin.order.routes.js` (route admin, qui a sa propre requête SQL indépendante — join `code_promo` ajouté séparément, sans quoi la route admin restait muette sur la remise malgré le fix service). Vérifié visuellement dans le navigateur sur les deux pages.
- **Fichiers modifiés :** `backend/src/services/invoice.service.js`, `backend/src/services/invoice-pdf.generator.js`, `backend/src/repositories/invoice.repository.js`, `backend/src/repositories/order.repository.js`, `backend/src/routes/admin.order.routes.js`, `backend/scripts/migrations/0014_facture_remise.sql`, `backend/scripts/init.sql`, `backend/tests/unit/invoice.service.test.js`, `frontend/src/pages/OrderConfirmationPage.jsx`, `frontend/src/pages/admin/AdminOrderDetail.jsx`. Migration 0014 appliquée en local dev et en production (`jana-db`).
