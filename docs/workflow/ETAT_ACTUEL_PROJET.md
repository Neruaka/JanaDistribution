...
| Dernier commit | Phase 17 (T17-01..T17-04) : correctifs images/bandeau Navbar + recadrage visuels catalogue + layout fiche produit centré |
| Fichiers modifiés non commités | Aucun |
| Phase active | Phase 17 — retours de la troisième phase de tests utilisateur, **TERMINÉE** (4/4, T17-01..T17-04 DONE). Phase 16 reste COMPLÈTE (13/13). |
...
| Verdict | **EN PRODUCTION SUR FLY.IO et fonctionnel, facturation légale complète** (T5-14 immuabilité + T5-15 avoir + T15-03 devis + T16-12 remise code promo). Backlog de code Phase 17 **entièrement traité** (4/4). Validations légale et comptable désormais **confirmées** (2026-09-08). DB-04 : rotation Gmail confirmée le 2026-09-05, purge de l'historique git explicitement reportée (décision propriétaire, 2026-09-08) — n'est plus un blocage ouvert. Reste : décision propriétaire sur la mise à jour des 189 visuels déjà en ligne avec leur version recadrée (T17-01). |
...
| 2026-09-08 | Phase 17 — Retours de la troisième phase de tests utilisateur + clôture des validations externes | **T17-01** : `ProductDetailPage.jsx` — `object-cover` → `object-contain` (desktop + mobile), cadre image desktop fixé à 637×637px (remplace `max-h-[420px] aspect-square`), sur demande explicite du propriétaire. Correction ultérieure le même jour après retour propriétaire sur le rendu réel (capture à l'appui) : un premier essai de recadrage automatique côté client (`ProductImageFrame.jsx`, détection de zone utile par différence de couleur) a été **abandonné** (détection peu fiable sur les dégradés doux des photos studio) puis supprimé ; diagnostic correct obtenu en comparant directement le fichier source au rendu — les photos étaient déjà bien cadrées, le vrai problème était le layout (voir T17-04). Une passe de recadrage serveur one-shot (`sharp trim`, vérifiée visuellement) a quand même été appliquée aux 188 visuels de `jana-catalogue-images/` (0 à 42 % de fond neutre en moins selon la photo, jamais de rognage du produit). **T17-02** : bandeau utilitaire `Navbar.jsx` — contenu transformé en défilement continu (`marquee-scroll`, `frontend/src/index.css`, pause au survol, désactivé si `prefers-reduced-motion`, doublon `aria-hidden`), couleur `text-mist-2` → `text-mist` pour plus de lisibilité sur fond `ink-900`. **T17-03** : 188 visuels catalogue réel (`.webp`) versionnés dans `jana-catalogue-images/` — constat que les 189 produits déjà en ligne utilisent déjà ces mêmes visuels (uploadés séparément via l'admin, correspondance confirmée par hash MD5). **T17-04** : `ProductDetailPage.jsx` — grid `md:grid-cols-[1fr_430px]` (colonne image flexible) remplacé par `md:grid-cols-[637px_430px] md:justify-center` (largeur fixe + grid centré) pour supprimer le grand vide entre l'image et le bloc prix sur écran large ; même correctif appliqué au squelette de chargement. **Hors code :** validations légale (T9-03) et comptable (T9-04/DB-03) confirmées par le propriétaire ; vraies informations d'entreprise (SIRET `79878778400015`, adresse `1 ESPLANADE DE COLLIOURE 93330 NEUILLY-SUR-MARNE`, TVA `FR92798787784`, vérifiées via annuaire-entreprises.data.gouv.fr) posées en secrets Fly `ENTREPRISE_SIRET`/`ENTREPRISE_ADRESSE`/`ENTREPRISE_TVA_NUMERO` sur `jana-backend`, backend redéployé et healthcheck vérifié ; décision propriétaire de reporter la purge de l'historique git (DB-04). **Reste ouvert :** les 189 visuels déjà en ligne (copies séparées sur le volume Fly, même hash que les fichiers du dépôt avant recadrage) n'ont pas été mis à jour avec leur version recadrée — un remplacement direct par SSH sur le volume a été bloqué par le classificateur de sécurité de la session ; décision propriétaire en attente (SSH direct autorisé, ré-upload manuel via l'admin, ou statu quo). | `npm run build` (frontend) ✓ à chaque étape | DONE |
...
**Session Phase 17 (2026-09-08) : les 4 retours remontés lors de la troisième phase de tests utilisateur sont corrigés (image produit — redimensionnement propre + cadre desktop 637×637, puis correction du vrai problème signalé — layout centré, voir T17-01/T17-04 ; bandeau utilitaire — défilant + couleur plus lisible ; visuels catalogue réel recadrés et versionnés). En parallèle, les 3 validations externes qui restaient bloquantes sont désormais levées : validation juridique (CGV/RGPD, T9-03), validation comptable TVA (T9-04/DB-03) et mentions d'entreprise réelles sur facture (SIRET/adresse/TVA, DM-06) — toutes confirmées par le propriétaire et posées en production. Décision propriétaire sur DB-04 : la purge de l'historique git est reportée pour l'instant (rotation des secrets déjà effective, pas de risque d'exploitation active). Reste ouverte : décision propriétaire sur la mise à jour des 189 visuels déjà en ligne avec leur version recadrée (SSH direct sur le volume Fly bloqué par le classificateur de sécurité de la session).**
...
| Champ | Valeur |
|---|---|
| Option A | Décider comment répercuter le recadrage sur les 189 visuels déjà en ligne (SSH direct sur le volume Fly, ré-upload manuel via l'admin, ou statu quo) |
...
| Prérequis option A | Décision propriétaire (voir T17-01) |
| Prérequis option B | Aucun |
| Prérequis option C | Aucun |

**Actions restantes avant une prod pleinement finalisée :**
1. Décider comment répercuter le recadrage d'image (T17-01) sur les 189 produits déjà en ligne
