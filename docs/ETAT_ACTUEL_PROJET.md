...
| Fichiers modifiés non commités | `docs/DEPLOY-HOMESERVER.md` (nouveau), `docs/DEPLOY-RAILWAY.md`/`docs/RAILWAY_CONFIG_READY.md` (bandeau obsolescence), `CLAUDE.md`/`docs/ETAT_ACTUEL_PROJET.md`/`docs/PLAN_CORRECTION_AUDIT.md` (Phase 11), `.github/workflows/deploy.yml`, suppression `backend/railway.json` + `frontend/railway.json` |
...
Détails complets : `docs/DEPLOY-HOMESERVER.md`. Railway (NIXPACKS backend, Dockerfile frontend) reste actif en parallèle jusqu'au cutover (T11-09), voir `docs/DEPLOY-RAILWAY.md` (obsolète, référence historique).
...
| 2026-06-14 | Plan opérationnel | Création de docs/CLAUDE_WORKFLOW.md, docs/ETAT_ACTUEL_PROJET.md, docs/PLAN_CORRECTION_AUDIT.md | — | DONE |
...
| 2026-06-28 | Docker | docker-compose.yml remplacé : Dockerfile.dev backend (nodemon) + frontend (Vite --host), DB_HOST=postgres overridé pour la logique dev. docs/DEMARRAGE.md créé. start-local.bat/.sh supprimés. | — | DONE |
...
2. Ajouter les 2 public hostnames Cloudflare Tunnel (T11-04) — voir `docs/DEPLOY-HOMESERVER.md`
