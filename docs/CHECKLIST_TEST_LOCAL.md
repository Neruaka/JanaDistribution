...
> statut positionné manuellement par un admin. Voir `ETAT_ACTUEL_PROJET.md` pour
...
cd backend && npm test
```
- [ ] 112/112 tests passés (référence de base — peut être plus élevé si des tests ont été ajoutés depuis, voir `PLAN_CORRECTION_AUDIT.md` pour le dernier total connu)
...
| Tests backend (mockés) | voir `PLAN_CORRECTION_AUDIT.md` pour le dernier total connu |
